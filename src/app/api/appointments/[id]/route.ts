    ...
    if (supabaseUrl && supabaseServiceKey && body.status && body.status !== currentAppointment.status) {
      ...
      try {
        if (body.status === "cancelled") {
          await checkTriggers("booking_cancelled", triggerData, serviceClient, profile.tenant_id, user.id);
        } else if (body.status === "rescheduled") {
          await checkTriggers("booking_rescheduled", triggerData, serviceClient, profile.tenant_id, user.id);
        }
      } catch (err) {
