...
import { PublicContractView } from "@/components/contract/PublicContractView";
...
export default function PublicContractPage({
  ...
  useEffect(() => {
    async function fetchContract() {
      try {
        const res = await fetch(`/api/contracts/public/${slug}`);
        if (!res.ok) {
          setNotFound(true);
          return;
        }
      ...
      <div className="dark relative min-h-screen bg-gray-950 flex items-center justify-center">
        ...
        <div className="relative z-10 text-center">
          ...
          <p className="mt-2 text-gray-400">
            This contract could not be found or is no longer available.
          </p>
        </div>
      </div>
    );
      ...
      <main className="relative z-10 flex-1 py-8 px-4 sm:px-6">
        <div className="mx-auto max-w-4xl">
          <div className="rounded-2xl border border-white/[0.08] bg-gray-900/80 backdrop-blur-sm overflow-hidden">
            <PublicContractView contract={contract} />
          </div>
        </div>
      </main>
